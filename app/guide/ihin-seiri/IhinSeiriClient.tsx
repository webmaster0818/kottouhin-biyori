...
const katamiwake = [
  { title: "形見分けは相続の話と切り離さない", text: "形見分けは「気持ちの整理」として行われますが、骨董品のように金銭的価値のある品は、渡した時点で遺産分割の結果に影響します。誰がどれを受け取るかを決める前に、おおよその価値を把握しておくと、後から不公平感が生まれるのを防げます。価値がはっきりしない品は、形見分けの対象に入れる前に査定を受けておくのが安全です。" },
  { title: "渡す相手と時期の目安", text: "形見分けは四十九日法要や一周忌など、親族が集まるタイミングに合わせて行われるのが一般的です。故人と縁の深かった方へ渡すのが慣例ですが、目上の方へ一方的に贈るのは避けるという考え方もあります。遠方の親族がいる場合は、写真を共有して希望を聞いてから決めると、後日の行き違いを減らせます。" },
  { title: "受け取る側の負担も考える", text: "掛軸・屏風・大きな壺などは、受け取った側に保管場所と管理の手間が生じます。桐箱ごと渡す、保管方法を書いた紙を添える、といった配慮があると引き継ぎやすくなります。受け取りを断られた品は、売却に回すか残すかを改めて相続人全員で決めましょう。" },
  { title: "記録を残す", text: "誰が何をいつ受け取ったかを一覧にして残しておくと、遺産分割協議書を作る際にも、後から「聞いていない」という話が出た際にも役立ちます。品名・写真・受け取った人・日付の4項目で十分です。" },
];

const familyCoordination = [
  { title: "最初に「全体像」を共有する", text: "親族間でもめる原因の多くは、情報の非対称です。実家に近い人だけが現物を見ている状態だと、遠方の親族は判断ができず不信感につながります。まず全点を撮影して一覧にし、相続人全員が同じ資料を見られる状態を作ってから話し合いを始めてください。" },
  { title: "「売る・残す・譲る」を先に決める", text: "個別の品ごとに議論を始めると論点が散らかります。先に方針として、売却に回す範囲・家に残す範囲・形見分けに回す範囲の3つの枠を決め、そのうえで品物を振り分けると合意が早まります。" },
  { title: "査定は全員が把握できる形で受ける", text: "一人が単独で査定を受けて金額だけ報告すると、額の妥当性をめぐって疑念が残りがちです。査定結果は明細を残してもらい、可能であれば複数社の結果を並べて共有しましょう。金額は作家・状態・時期により異なるため、差が出ること自体は珍しくありません。" },
  { title: "取り分でもめそうなときは書面にする", text: "売却代金の分配方法まで含めて遺産分割協議書に記載しておくと、後々の争いを防げます。相続人同士で折り合いがつかない場合は、早い段階で弁護士や司法書士に入ってもらうほうが結果的に費用も時間も抑えられます。" },
  { title: "急かさない・急かされない", text: "退去日や法要の日程が迫っていると、誰かが独断で処分を進めてしまいがちです。期限が厳しい場合でも、価値の可能性がある品だけは別に保管して判断を先送りできるようにしておくと、取り返しのつかない事態を避けられます。" },
];
...
const faqItems = [
  ...
  { q: "形見分けで骨董品を渡すとき、注意することはありますか？", a: "金銭的な価値がある品を渡すと、遺産分割の結果に影響します。誰がどれを受け取るかを決める前におおよその価値を把握し、価値がはっきりしない品は先に査定を受けてください。受け取る側には保管場所と管理の手間が生じるため、桐箱ごと渡す、保管方法を添えるといった配慮があると引き継ぎやすくなります。品名・写真・受け取った人・日付を一覧にして残しておくと後の手続きにも役立ちます。" },
  { q: "遺品整理はいつまでに終わらせるべきですか？", a: "法律上の期限は遺品整理そのものにはありませんが、相続放棄を検討する場合は相続開始を知ってから3ヶ月以内、相続税の申告は相続開始を知った日の翌日から10ヶ月以内という期限があります。賃貸物件の退去日や法要の日程が先に決まっていることも多いため、価値の可能性がある品だけは別に保管し、判断を急がなくてよい状態を作っておくと安心です。" },
  { q: "親族が遠方にいて話がまとまりません。どう進めればよいですか？", a: "もめる原因の多くは情報の非対称です。まず全点を撮影して一覧を作り、相続人全員が同じ資料を見られる状態にしてください。そのうえで「売る・残す・譲る」の3つの枠を先に決め、品物を振り分けると合意が早まります。査定は明細を残してもらい、可能であれば複数社の結果を並べて共有しましょう。" },
  { q: "残すと決めた品以外は、どう処分すればよいですか？", a: "買取・オークション・寄付譲渡・自治体での廃棄・整理業者への依頼という5つの選択肢があり、品物の価値・量・急ぎ度で適した方法が変わります。選択肢の比較、実際に捨てるときの分別と手順、ごみに出せない品目の扱いについては、骨董品の処分方法・捨て方のページにまとめています。" },
];
...
遺品整理の進め方・形見分け・親族間の調整から相続手続きまでの完全ガイド
            </p>
          </div>
        </section>
          ...
          <div className="max-w-4xl mx-auto px-4">
            ...
            <div className="bg-[#FAF7F2] border-2 border-[#C9A96E]/50 rounded-2xl p-5 md:p-6 mb-8">
              ...
              <p className="text-sm md:text-base text-[#5C4A3A] leading-relaxed mt-3">
                <strong className="text-[#2C1810]">このページの守備範囲：</strong>
                遺品整理としての進め方（スケジュール・形見分け・親族間の調整・相続と税）を扱います。
                遺品整理に限らず「骨董品をどう処分するか」を、買取・オークション・寄付譲渡・自治体での廃棄・整理業者の5通りで比較し、
                実際に捨てるときの分別・手順やごみに出せない品目までまとめたものは
                <Link href="/learn/how-to-dispose/" className="text-[#8B4513] font-bold underline underline-offset-2 hover:text-[#6B3410]">骨董品の処分方法・捨て方</Link>
                にあります。生前整理・蔵整理でまとめて出てきた場合の進め方もそちらにまとめています。
              </p>
            </div>
        ...
        <section className="py-12 md:py-16 bg-white">
          <div className="max-w-4xl mx-auto px-4">
            <div className="text-center mb-10">
              <p className="text-sm text-[#C9A96E] tracking-widest mb-2">KATAMIWAKE</p>
              <h2 className="font-serif-jp text-2xl md:text-3xl font-bold text-[#2C1810]">
                形見分けの進め方と、骨董品を渡すときの注意点
              </h2>
              <p className="text-[#5C4A3A] mt-2">遺品整理でいちばん気を使う「誰に何を渡すか」の決め方</p>
            </div>
            <div className="space-y-6">
              {katamiwake.map((item, idx) => (
        ...
        <section className="py-12 md:py-16">
          <div className="max-w-4xl mx-auto px-4">
            <div className="text-center mb-10">
              <p className="text-sm text-[#C9A96E] tracking-widest mb-2">FAMILY</p>
              <h2 className="font-serif-jp text-2xl md:text-3xl font-bold text-[#2C1810]">
                相続人が複数いる場合の親族間の調整
              </h2>
              <p className="text-[#5C4A3A] mt-2">遺品整理でもめないための進め方と、決めておくべきこと</p>
            </div>
            <div className="space-y-4">
              {familyCoordination.map((item, idx) => (
                <div key={idx} className="bg-white rounded-xl border border-[#E0D5C8] p-6 shadow-sm">
                  <div className="flex gap-4 items-start">
            ...
            <div className="mt-6 bg-[#F5ECD7] rounded-xl border border-[#C9A96E] p-5">
              <p className="text-sm text-[#5C4A3A] leading-relaxed">
                なお、話し合いの結果「売らずに処分する」と決まった品の扱い方
                ——買取・オークション・寄付譲渡・自治体での廃棄・整理業者への依頼という
                5つの選択肢の比較と、実際に捨てるときの分別・手順——については、
                <Link href="/learn/how-to-dispose/" className="text-[#8B4513] font-bold underline underline-offset-2 hover:text-[#6B3410]">骨董品の処分方法・捨て方</Link>
                で詳しく解説しています。このページでは遺品整理としての進め方に絞ってご案内しています。
              </p>
            </div>
          </div>
        </section>
